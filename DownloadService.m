...
- (BOOL)getFileInfo {
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error) {
            ...
            NSString *oEtag = self.checkpoint.etag;
            if ([oEtag isEqualToString:etag]) {
                resumable = YES;
            } else {
                resumable = NO;
            }
...
- (void) getEtag {
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error) {
            ...
            self.checkpoint.etag = etag;
            self.checkpoint.totalExpectedLength = len;
        }
        dispatch_semaphore_signal(self.semaphore);
    }];
    [task resume];
    ...
    if ([httpResponse isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if (error) {
            if (self.failure) {
                ...
                [userInfo setObject:self.checkpoint forKey:@"checkpoint"];
                NSError *tError = [NSError errorWithDomain:error.domain code:error.code userInfo:userInfo];
                self.failure(tError);
            }
        } else if (self.success) {
            self.success(@{@"statusCode":@(httpResponse.statusCode),@"bytesWritten":@(self.checkpoint.totalExpectedLength)});
        }
